...
Kage is a minimalist yet powerful web framework for Deno. Write APIs with full
type safety, built-in validation, and zero hidden abstractions.

> **Early Development:** Kage is in active development and not yet
> production-ready.
...
1. **Explicit over implicit** - No magic decorators or hidden dependency
   injection
...
- **Context Extensions** - Extend handlers with `decorate`, `derive`, and
  `state`
- **Plugins** - Composable functions that use context extensions
- **Lifecycle Hooks** - `onRequest`, `onBeforeHandle`, `onAfterHandle`,
  `onResponse`
...
- **Status Helpers** - Convenient methods like `unauthorized()`, `forbidden()`,
  `notFound()`
...
- [Plugins](/docs/advanced/plugins) - Create reusable extensions with
  `decorate`, `derive`, `state`
